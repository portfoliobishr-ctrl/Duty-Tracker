...
export const ImamDutyView: React.FC = () => {
  ...
  const refreshData = () => {
    const data = dutyStore.getDualPoolStats();
    setDualStats(data);
  };
  ...
  const handleToggleStudentTurn = (student: Student, pool: PoolType, currentStatus: boolean, roundNumber: number) => {
    dutyStore.toggleStudentRoundTurn(student.id, !currentStatus);
    if (!currentStatus) {
      try {
        confetti({
          particleCount: 35,
          ...
          <div className="flex items-start justify-between gap-2 pb-4 border-b border-slate-100">
            <div>
              ...
              <h3 className="text-base font-bold text-slate-900 mt-1.5">
                {isRegular ? 'Regular Students Queue' : 'College Students Queue'}
              </h3>
              <p className="text-xs text-slate-500 mt-0.5">
                {isRegular
                  ? 'Assigned on regular working days. 10 students in custom rotation order.'
                  : 'Assigned on weekends (Saturdays & Sundays) & holidays. 6 students in custom rotation order.'}
              </p>
            </div>
          </div>
  ...
  return (
    <div className="space-y-4 sm:space-y-5 max-w-5xl mx-auto pb-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <div className="inline-flex items-center space-x-1.5 px-2.5 py-0.5 rounded-full bg-amber-50 text-amber-800 text-xs font-bold mb-1 border border-amber-200/60">
              ...
              <span>Asr Prayer Congregational Imam Duty</span>
            </div>
            <h2 className="text-lg font-bold text-slate-900 tracking-tight">
              Dual-Pool Alphabetical Rotation Roster
            </h2>
            <p className="text-xs text-slate-500 max-w-2xl mt-0.5">
              Imam duty applies strictly to the daily Asr prayer. College students attend college on weekdays, so they rotate on weekends (Saturdays &amp; Sundays) &amp; holidays (Pool B, 6 students). Regular students rotate on working weekdays (Pool A, 10 students). Both queues strictly follow alphabetical order (A to Z).
            </p>
          </div>
      ...
      <div className={`grid gap-4 sm:gap-5 ${
        ...
        {(activeTab === 'all' || activeTab === 'regular') && renderQueueSection(poolA)}
        {(activeTab === 'all' || activeTab === 'college') && renderQueueSection(poolB)}
      </div>
